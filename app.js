/**
 * WSS Panel Frontend (Axiom Refactor V8.0 - Full TG Feature UI)
 *
 * [AXIOM V11.0.1 CHANGELOG]
 * - [REVERT] 恢复 CORE_SERVICES_MAP 对 'udpgw' (BadVPN) 的支持。
 * - [FIX] 修复 fetchAllStaticData 中服务日志按钮的生成逻辑，确保所有 CORE_SERVICES_MAP 中的服务都被渲染。
 * - [UPDATE] 扩展 switchView 逻辑，在切换到 'vouchers' 视图时自动加载卡密列表。
 * - [UI FIX] 更新 CORE_SERVICES_MAP，使 'udpgw' 和 'wss-udp-custom' 同时存在。
 */
...
let panelSocket = null; // WebSocket 实例
let wsReconnectTimer = null; // WS 重连定时器

let lastUserStats = { total: -1, total_traffic_gb: -1 };

const TOKEN_PLACEHOLDER = "[*********]";

// [AXIOM V11.0.1 REVERT] 恢复 udpgw 的支持
...
function switchView(viewId) {
    // [V11.0 UPDATE] 新增 'vouchers' 视图
    const views = ['dashboard', 'users', 'settings', 'security', 'live-ips', 'hosts', 'payload-gen', 'port-config', 'vouchers'];
    views.forEach(id => {
...
function renderSystemStatus(data) {
    ...
    // [V11.0.1 FIX] 确保所有 CORE_SERVICES_MAP 中的服务都能够被渲染，并使用后端返回的 data.services 状态
    Object.keys(CORE_SERVICES_MAP).forEach(key => {
        const item = data.services[key] || { name: CORE_SERVICES_MAP[key], status: 'unknown' };
...
function renderVouchersList(vouchers) {
    ...
    if (vouchers.length === 0) {
        tbody.innerHTML = '<tr><td colspan="5" class="px-6 py-4 text-center text-gray-500">没有找到卡密。</td></tr>';
        return;
    }

    tbody.innerHTML = vouchers.map(v => {
        const statusText = v.is_used === 1 ? `已使用 (${v.used_by})` : '未使用';
        const statusColor = v.is_used === 1 ? 'text-error' : 'text-success';
        const usedTime = v.is_used === 1 ? v.used_at.substring(0, 16) : 'N/A';
        
        return `
            <tr class="hover">
                <td class="font-mono text-sm text-primary cursor-pointer" onclick="copyToClipboard(null, '卡密 ${v.code} 已复制')">${v.code}</td>
                <td>${v.days} 天</td>
                <td>${v.quota_gb} GB</td>
                <td class="${statusColor}">${statusText}</td>
                <td class="text-xs text-gray-500">${usedTime}</td>
            </tr>
        `;
    }).join('');
}
    ...
    try {
        ...
        if (!contentType || !contentType.includes("application/json")) {
            if (response.ok) {
                ...
                showStatus("API 响应格式错误：非 JSON 页面。", false);
                return null;
            }
        }
...
async function fetchServiceLogs(serviceId) {
    ...
    if (data && data.logs) {
        ...
        logContainer.textContent = `~$ 错误: 无法获取 ${serviceName} 日志. ${data ? data.message : '未知错误'}`;
    }
}
...
// [V11.0 UPDATE] 扩展以读取 tg_alarm_threshold
async function fetchGlobalSettings() {
     ...
     if (data && data.settings) {
        ...
        document.getElementById('global-tg-alarm-threshold').value = data.settings.tg_alarm_threshold || 90;
     }
}

// [V11.0 UPDATE] 扩展以保存 tg_alarm_threshold
async function saveGlobalSettings() {
    const fuseThreshold = document.getElementById('global-fuse-threshold').value;
    const tgAlarmThreshold = document.getElementById('global-tg-alarm-threshold').value;
    ...
    const result = await fetchData('/settings/global', {
        ...
        body: JSON.stringify({ 
            ...
            tg_alarm_threshold: parseInt(tgAlarmThreshold)
        })
    });
...
async function fetchGlobalConfig() {
     ...
     if (data && data.config) {
        ...
        // [V11.0.1 RESTORE] 填充 BadVPN UDPGW 端口
        if (document.getElementById('config-udpgw-port')) {
            document.getElementById('config-udpgw-port').value = data.config.udpgw_port || 7300;
        }
        // [AXIOM V5.7] 填充 UDP Custom 端口
        if (document.getElementById('config-udp-custom-port')) {
...
async function saveGlobalConfig() {
    ...
    const configData = {
        ...
        // [V11.0.1 RESTORE] 读取 BadVPN UDPGW 端口
        udpgw_port: parseInt(document.getElementById('config-udpgw-port').value) || 7300,
        // [AXIOM V5.7] 读取 UDP Custom 端口
...
async function fetchVouchers() {
    const data = await fetchData('/vouchers/list');
    if (data && data.vouchers) {
        renderVouchersList(data.vouchers);
    } else {
        renderVouchersList([]);
    }
}
...
async function createVoucher() {
    const daysEl = document.getElementById('voucher-days');
    const quotaGbEl = document.getElementById('voucher-quota-gb');
    
    const days = parseInt(daysEl.value) || 0;
    const quotaGb = parseFloat(quotaGbEl.value) || 0.0;
    
    if (days <= 0 && quotaGb <= 0) {
        showStatus('天数和流量至少有一项大于 0', false);
        return;
    }
    
    showStatus('正在创建卡密...', true);

    const result = await fetchData('/vouchers/create', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ days, quota_gb: quotaGb })
    });
    
    if (result && result.success) {
        showStatus(`卡密 ${result.code} 创建成功!`, true);
        daysEl.value = 0;
        quotaGbEl.value = 0;
        fetchVouchers(); 
    } else if (result) {
        ...
        try {
            ...
            switch (message.type) {
                ...
                case 'hosts_changed':
                    ...
                    break;
                case 'vouchers_changed': // [V11.0 NEW] 
                    if (currentView === 'vouchers') {
                        console.log('[AXIOM V11.0] 收到 vouchers_changed 推送，正在刷新卡密列表...');
                        fetchVouchers();
                    }
                    break;
    ...
    try {
        ...
        if (data && data.config) {
            FLASK_CONFIG = {
                ...
                UDPGW_PORT: data.config.udpgw_port, // [V11.0.1 RESTORED]
        ...
        if (currentView === 'vouchers') { fetchVouchers(); } // [V11.0 NEW]
        ...
        if (btnGroup) {
            ...
            // [V11.0.1 FIX] 确保服务日志按钮的顺序和名称与 CORE_SERVICES_MAP 匹配
            Object.keys(CORE_SERVICES_MAP).forEach(key => {
                const serviceName = CORE_SERVICES_MAP[key];
                const newButton = document.createElement('button');
                newButton.setAttribute('onclick', `fetchServiceLogs('${key}')`);
                newButton.className = 'btn btn-ghost btn-sm';
                newButton.textContent = serviceName;
