...
# ==========================================================
# WSS 隧道与用户管理面板 一键卸载脚本 (V2.4 - Binary UDP Restore)
# ==========================================================
...
systemctl stop udpgw || true # [RESTORED]
systemctl disable udpgw || true
systemctl stop wss-udp-custom || true
systemctl disable wss-udp-custom || true
...
rm -f "$UDPGW_SERVICE_PATH" # [RESTORED]
rm -f "$UDP_CUSTOM_SERVICE_PATH" 
...
# 3. 清理 IPTABLES 规则
# =============================
echo "3. 清理 IPTABLES 规则链 ($BLOCK_CHAIN, $UDP_REDIR_CHAIN)..."
...
if [ -f "$IPTABLES_RULES" ]; then
    ...
    /sbin/iptables-save | grep -v "$BLOCK_CHAIN" | grep -v "$UDP_REDIR_CHAIN" | /sbin/iptables-restore || true
fi
...
rm -f "$WSS_PROXY_PATH"
rm -f "$UDPGW_BIN_PATH" # [RESTORED]
rm -f "$UDP_CUSTOM_BIN_PATH"
